...
exec_cmd("localtonet --help")
exec_cmd("ls -la ~/.config || true")
exec_cmd("find ~ -name '*.json' -o -name '*token*' -o -name '*localto*' 2>/dev/null | grep -v 'venv' | grep -v 'site-packages' || true")
s.close()
