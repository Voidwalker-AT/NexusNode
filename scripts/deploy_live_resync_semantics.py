"""
NexusNode — Deploy Live Resync Semantics & Validation to TECNO BG6
"""
...
FILES_TO_SYNC = [
    ...
    "tests/test_live_resync_semantics.py"
]
...
def main():
    print("=== Deploying Live Resync Semantics & Safety Guards to TECNO BG6 ===")
    ...
    ts = int(time.time())
    backup_dir = f"{REMOTE_BASE}/backups/resync_{ts}"
    run_cmd(ssh, f"mkdir -p {backup_dir}/agent {backup_dir}/static/js {backup_dir}/tests")
    ...
    # 5. Check health endpoint on appliance
    print("Checking internal health endpoint...")
    out, _ = run_cmd(ssh, "curl -s http://127.0.0.1:5000/api/health")
    print(f"Internal Health: {out}")

    # 6. Run new live resync test suite directly on BG6
    print("Running test_live_resync_semantics.py directly on BG6...")
    out, err = run_cmd(ssh, f"cd {REMOTE_BASE} && python -m unittest tests/test_live_resync_semantics.py")
    print(f"BG6 Test Output:\n{out}\n{err}")
    ...
    print(f"Active Chromium processes on BG6: {out.strip()}")

    ssh.close()

    # 8. Public HTTPS acceptance check
    print(f"\nVerifying public HTTPS endpoint ({PUBLIC_ORIGIN})...")
    try:
        r_health = requests.get(f"{PUBLIC_ORIGIN}/api/health", timeout=10)
        print(f"Public /api/health: HTTP {r_health.status_code} -> {r_health.json()}")

        r_tools = requests.get(f"{PUBLIC_ORIGIN}/api/mcp/tools", timeout=10)
        print(f"Public /api/mcp/tools: HTTP {r_tools.status_code} -> Found {len(r_tools.json().get('tools', []))} tools")
        
        # Verify academic.sync description on public surface
        tools = r_tools.json().get("tools", [])
        sync_tool = next((t for t in tools if t.get("name") == "academic.sync"), None)
        if sync_tool:
            print(f"[OK] academic.sync found in public catalog: {sync_tool.get('description')[:80]}...")
        else:
            print("[FAIL] academic.sync not found in public catalog!")

    except Exception as e:
        print(f"[WARN] Public HTTPS verification failed: {e}")

    print("\n=== Deployment & Remote Verification Complete ===")
